...
API_URL="${API_URL:-http://localhost:4022/api}"
PASS=0
FAIL=0
...
echo "=== AnvilPulse Integration Tests ==="
echo "API: $API_URL"
echo ""
...
LOGIN_RESPONSE=$(curl -s -w "\n%{http_code}" "$API_URL/auth/login" \
  ...
  -d '{"email":"demo@istanbuldemiratolyesi.com","password":"demo123456"}')
...
HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" "$API_URL/site-visits" -H "Authorization: Bearer $TOKEN")
assert_status "List Site Visits" 200 "$HTTP_CODE"

HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" "$API_URL/welding" -H "Authorization: Bearer $TOKEN")
assert_status "List Welding Tasks" 200 "$HTTP_CODE"

HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" "$API_URL/surface-finishes" -H "Authorization: Bearer $TOKEN")
assert_status "List Surface Finishes" 200 "$HTTP_CODE"

HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" "$API_URL/workshop" -H "Authorization: Bearer $TOKEN")
assert_status "Workshop Profile" 200 "$HTTP_CODE"

HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" "$API_URL/site-visits/revision" -H "Authorization: Bearer $TOKEN")
assert_status "Revision Site Visits" 200 "$HTTP_CODE"

CREATE_PROJECT=$(curl -s -w "\n%{http_code}" "$API_URL/projects" \
  -H "Authorization: Bearer $TOKEN" \
  -H 'Content-Type: application/json' \
  -d '{"projectNumber":"DA-9999","clientName":"Test Müşteri","projectType":"railing","quantity":10}')
